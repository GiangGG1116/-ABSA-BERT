...
from absa.data import ATEDataset, make_ate_collate
from absa.models import ATEBert
from absa.utils import avg, set_seed


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Train BERT for Aspect Term Extraction")
...
def main() -> None:
    ...
    train_ds = ATEDataset(train_df, tokenizer, max_length=cfg.max_length)
    valid_ds = ATEDataset(valid_df, tokenizer, max_length=cfg.max_length)

    collate_fn = make_ate_collate(tokenizer.pad_token_id)
    ...
    for epoch in range(1, cfg.epochs + 1):
        ...
        model.train()
        train_losses = []
        for ids, labels, masks in tqdm(train_loader, desc=f"[ATE Train] epoch {epoch}"):
        ...
        eval_losses = []
        with torch.no_grad():
            for ids, labels, masks in tqdm(valid_loader, desc=f"[ATE Eval]  epoch {epoch}"):
                ids, labels, masks = ids.to(device), labels.to(device), masks.to(device)
                loss, _ = model(ids, masks, labels)
        ...
        print(f"Epoch {epoch:02d} | train_loss={avg_train:.4f}  valid_loss={avg_eval:.4f}")
